...
const BOT_TOKEN = "8732557330:AAHhcI0Lx1wicMArc4z7ON-B070AzR8_Ct0";//*replace your bot token
const ADMIN_IDS = [8268479241, 7915159454];
...
function loadUserSettings() {
 ...
 function convertToPremiumEmojis(text) {
    ...
    let result = text;
    for (const [emoji, premiumId] of Object.entries(PREMIUM_EMOJI_MAP)) {
        const regex = new RegExp(emoji.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
        const premiumTag = `<tg-emoji emoji-id="${premiumId}">${emoji}</tg-emoji>`;
        result = result.replace(regex, premiumTag);
    }
    return result;
}

 async function sendPremiumMessage(ctx, text, replyMarkup = null, extraOptions = {}) {
    const premiumText = convertToPremiumEmojis(text);
    ...
    try {
        await ctx.reply(premiumText, options);
    } catch (error) {
        console.error('Premium emoji send error:', error.message);
 ...
 async function sendMessageWithRetry(ctx, text, replyMarkup = null) {
    for (let attempt = 0; attempt < MAX_TELEGRAM_RETRIES; attempt++) {
        try {
            await sendPremiumMessage(ctx, text, replyMarkup);
            return true;
        } catch (error) {
            ...
            try {
                const premiumText = convertToPremiumEmojis(text);
                await ctx.reply(premiumText, { reply_markup: replyMarkup });
                return true;
            } catch (e) {
                await ctx.reply(text, { reply_markup: replyMarkup });
                return false;
...
// Images URL
const PROFIT_TEMPLATE_URL = "https://i.ibb.co/XfM1ztdN/IMG-20260627-181957-126.jpg";
const LOSS_TEMPLATE_URL = "https://i.ibb.co/39D10G62/IMG-20260627-182223-731.jpg";
...
async function placeManualBet(userId, session, betChoice, gameType, ctx) {
  try {
    ...
    await sendPremiumMessage(ctx, `❌ Bet error: ${error.message}`);
  }
}
...
async function processManualBetAmount(userId, betAmount, ctx, betChoice, gameType) {
  try {
    ...
    if (!session) {
      await sendPremiumMessage(ctx, "❌ Please login first");
   ...
   let currentBalance = 0;
    try {
      ...
      await sendPremiumMessage(ctx, "❌ Failed to get balance");
      delete userState[userId];
      return;
    }
    ...
    if (betChoice === 'B' || betChoice === 'S') {
      ...
      await sendPremiumMessage(ctx, `❌ Invalid bet choice: ${betChoice}`);
      delete userState[userId];
      return;
    }
    ...
    if (!issueRes || issueRes.code !== 0) {
      await sendPremiumMessage(ctx, "❌ Failed to get current issue");
      delete userState[userId];
      return;
    }
    ...
    if (!currentIssue) {
      await sendPremiumMessage(ctx, "❌ Failed to get current issue number");
      delete userState[userId];
      return;
    }
    ...
    if (!selectType) {
      await sendPremiumMessage(ctx, `❌ Invalid bet selection: ${betChoice} for bet type: ${betType}`);
      delete userState[userId];
      return;
    }
    ...
    if (actualAmount === 0) {
      await sendPremiumMessage(ctx, `❌ Invalid bet amount: ${betAmount} Ks`);
      delete userState[userId];
      return;
    }
    ...
    if (balance === null || balance < actualAmount) {
      await sendPremiumMessage(ctx, `❌ Insufficient balance. Available: ${balance} Ks, Required: ${actualAmount} Ks`);
      delete userState[userId];
      return;
    }
    ...
    try {
      ...
      if (betResp.error || betResp.code !== 0) {
        ...
        await sendPremiumMessage(ctx, `❌ Bet failed: ${errorMsg}`);
        delete userState[userId];
        return;
      }
      ...
      await sendPremiumMessage(ctx, message, makeManualBetKeyboard());
  ...
  if (settings.strategy === "SNIPER") {
    ...
    if (stopLossLimit && currentProfit <= -stopLossLimit) {
      ...
      userPendingProfitStopMessages[userId] = {
        type: 'STOP_LOSS',
        message: convertToPremiumEmojis(
          `🔻 STOP LOSS LIMIT HIT\n\n` +
          `⚠️ SL SET: ${stopLossLimit} Ks\n` +
            ...
            if (settled && settled.number) {
                    ...
                    if (manualBets.length > 0) {
                      ...
                      if (imageBuffer) {
                        await bot.telegram.sendPhoto(userId, 
                          { source: imageBuffer },
                          {
                            caption: convertToPremiumEmojis(`📝 Recent Manual Bets - Last 10 bets`),
                            parse_mode: 'HTML'
                          }
                        );
                      }
              ...
              if (isLeslayStrategy) {
                if (settings.sniper_hit_twice) {
                  try {
                    const sniperMsg = "🎯 SNIPER HIT 2/2!✅ Target acquired successfully!🤖  Bot deactivated.";
                    await bot.telegram.sendMessage(userId, convertToPremiumEmojis(sniperMsg), { parse_mode: 'HTML' });
                  ...
                  try {
                    const firstHitMsg = "✅ First Target Hit!🎯 One more target to go!";
                    await bot.telegram.sendMessage(userId, convertToPremiumEmojis(firstHitMsg), { parse_mode: 'HTML' });
              ...
              if (!isLeslayStrategy) {
                if (entryLayer === 1) {
                  ...
                  if (isWin) {
                    ...
                    try {
                      await bot.telegram.sendMessage(userId, convertToPremiumEmojis(winMessage), { parse_mode: 'HTML' });
                      if (userPendingProfitStopMessages[userId]) {
                        const pendingMsg = userPendingProfitStopMessages[userId];
                        delete userPendingProfitStopMessages[userId];
                    ...
                    try {
                      await bot.telegram.sendMessage(userId, convertToPremiumEmojis(loseMessage), { parse_mode: 'HTML' });
                      if (userPendingProfitStopMessages[userId]) {
                        const pendingMsg = userPendingProfitStopMessages[userId];
                        delete userPendingProfitStopMessages[userId];
...
async function bettingWorker(userId, ctx, bot) {
  ...
  if (settings.virtual_mode) {
    ...
    if (!balanceRetrieved) {
      await sendPremiumMessage(ctx, "❌ Failed to check balance after multiple attempts. Please check your connection or try again.", makeMainMenuKeyboard(true));
      settings.running = false;
      return;
    }
  }
 ...
 const startMessage = 
  ...
  await sendPremiumMessage(ctx, startMessage);
...
function getCurrentYangonTime() {
  return moment().tz('Asia/Yangon').format('YYYY-MM-DD HH:mm:ss');
      ...
      if (currentBalance === null) {
        ...
        if (!recovered) {
          await sendPremiumMessage(ctx, "❌ Failed to recover balance. Stopping bot to prevent errors.", makeMainMenuKeyboard(true));
          settings.running = false;
          break;
        }
      }
      ...
      if (currentBalance < minBetSize) {
        ...
        await sendPremiumMessage(ctx, message, makeMainMenuKeyboard(true));
        settings.running = false;
        break;
      }
      ...
      if (currentBalance < balanceWarningThreshold && currentBalance >= minBetSize && (now - lastWarning > 60000)) {
        ...
        await sendPremiumMessage(ctx, warningMessage);
      ...
      if (shouldSkip) {
        ...
        await sendPremiumMessage(ctx, betMsg);
...
const displayProfit = Math.abs(totalProfit).toFixed(2);
...
if (settings.sniper_hit_twice) { 
  await sendMessageWithRetry(ctx, "🎯 SNIPER HIT 2/2!✅ Target acquired successfully!🤖  Bot deactivated.", makeMainMenuKeyboard(true)); 
  settings.sniper_hit_twice = false; 
} 
...
if (!userStopInitiated[userId]) {
  const message = `🍎 BOT DEACTIVATED\n\n` +
                `💳 ${balanceText}` +
                `💰 Total Profit: ${profitIndicator}${displayProfit} Ks`;
...
async function handleResultsCommand(ctx, userId) {
  ...
  if (imageBuffer) {
    await ctx.replyWithPhoto(
      ...
      await sendPremiumMessage(ctx, "❌ Failed to fetch results");
    }
  } catch (error) {
    logging.error(`Error fetching results: ${error.message}`);
    await sendPremiumMessage(ctx, "❌ Error fetching results");
  }
}
  ...
  try {
    ...
    if (photos.total_count > 0) {
      ...
      const welcomeKeyboard = Markup.inlineKeyboard([
        ...
        [Markup.button.url("📢 Join Updates Group", "https://t.me/devpaitrxsignal")]
      ]);
...
Developed by: @BLACK12566`;
      
      // ✅ Premium Emoji နဲ့ပို့မယ်
      await ctx.replyWithPhoto(fileId, {
        caption: convertToPremiumEmojis(welcomeCaption),
...
Developed by: @BLACK12566`;
      ...
      await sendPremiumMessage(ctx, welcomeMessage, welcomeKeyboard);
    }
  } catch (error) {
...
Developed by: @BLACK12566`;
  ...
  if (!loggedIn) {
    await sendMessageWithRetry(ctx, "Please login to continue:", makeMainMenuKeyboard(false, userId));
  } else {
    if (ADMIN_IDS.includes(userId)) {
      await sendMessageWithRetry(ctx, "Welcome back, Admin!", makeMainMenuKeyboard(true, userId));
    } else {
      await sendMessageWithRetry(ctx, "Welcome back!", makeMainMenuKeyboard(true, userId));
    }
  }
} 
...
async function cmdUsersHandler(ctx) {
  ...
  try {
    ...
    await sendPremiumMessage(ctx, "Error retrieving user list. Please try again later.");
  }
...
async function callbackQueryHandler(ctx) {
  ...
  if (data.startsWith("mode:")) {
    ...
    if (mode === "virtual") {
      ...
      await sendPremiumMessage(ctx, "💵 Switched to Real Mode", getKeyboardForCurrentMode(userId));
      saveUserSettings();
    }
...
if (data.startsWith("reactivate:")) {
  ...
  if (data.startsWith("sl_layer:")) {
    ...
    await sendPremiumMessage(ctx, `SL Layer : ${slValue} (${description})`, makeRiskControlKeyboard());
  ...
  if (data.startsWith("time_range:")) {
    ...
    if (action === "start") {
      ...
      await sendPremiumMessage(ctx, "✅ Time range cleared", makeRiskControlKeyboard());
      saveUserSettings();
    } else if (action === "show") {
      const now = new Date();
      const currentTime = formatTime(now);
...
async function showUserStats(ctx, userId) {
  try {
    ...
    if (settings.virtual_mode) {
      balance = stats?.virtual_balance || settings.virtual_balance || 0;
      startBalance = stats?.initial_balance || settings.virtual_balance || 0;
      profit = balance - startBalance;
    } else {
      balance = gameInfo?.balance || 0;
      profit = stats?.profit || 0;
      startBalance = stats?.start_balance || 0;
    }
    
    const profitSign = profit >= 0 ? '+' : '';
    
    let message = 
`📊 <b>Bot Configuration & Stats</b>
━━━━━━━━━━━━━━━━━

👤 <b>User Info</b>
├─ 🌐 Site: ${selectedSite}
├─ 🆔 User ID: ${gameInfo?.user_id || 'N/A'}
├─ 📛 Username: ${gameInfo?.username || 'N/A'}
├─ 📛 Nickname: ${gameInfo?.nickname || 'N/A'}
└─ 💰 Balance: ${balance.toFixed(2)} Ks

🎮 <b>Game Settings</b>
├─ 🎲 Game Type: ${settings.game_type || 'TRX'}
├─ 🎯 Bet Type: ${settings.bet_type === 'COLOR' ? 'Color' : 'Big/Small'}`;

    // Strategy
    let strategyDisplay = settings.strategy || 'AI_PREDICTION';
    const strategyMap = {
      'AI_PREDICTION': 'CHAT GPT V1',
      'LYZO': 'LYZO',
      'DREAM': 'DREAM',
      'BABIO': 'BABIO',
      'BS_ORDER': 'BS-Order',
      ...
      'SNIPER': 'LESLAY',
      'ALINKAR': 'ALINKAR',
      'MAY_BARANI': 'MAY BARANI',
      'BEATRIX': 'BEATRIX'
    };
    strategyDisplay = strategyMap[strategyDisplay] || strategyDisplay;
    
    message += `\n├─ 📚 Strategy: ${strategyDisplay}`;
    
    // Betting Strategy
    const bettingStrategyDisplay = settings.betting_strategy || 'Martingale';
    message += `\n└─ 🕹️ Betting: ${bettingStrategyDisplay}`;

    // Bet Sizes
    if (settings.bet_sizes && settings.bet_sizes.length > 0) {
      message += `\n\n💰 <b>Bet Wrager</b>
└─ ${settings.bet_sizes.join(' - ')} Ks`;
    }
    ...
    let entryDisplay = layerLimit;
    if (isLeslayStrategy) entryDisplay = 'Disabled (Leslay)';
    
    let slDisplay = slLayer || 'Not set';
    if (isLeslayStrategy) slDisplay = 'Disabled (Leslay)';
    
    message += `\n\n⚙️ <b>Risk Control</b>
├─ ⛳ Entry Layer: ${entryDisplay}
├─ 💥 SL Layer: ${slDisplay}
├─ 🔺 Profit Target: ${isLeslayStrategy ? 'Disabled (Leslay)' : (profitTarget ? profitTarget + ' Ks' : 'Not set')}
└─ 🔻 Stop Loss Limit: ${stopLoss ? stopLoss + ' Ks' : 'Not set'}`;

    const modeText = settings.virtual_mode ? '🖥️ Virtual Mode' : '💵 Real Mode';
    message += `\n\n🎛️ <b>Mode</b>
└─ ${modeText}`;

    const statusText = settings.running ? '🟢 Running' : '🔴 Stopped';
    message += `\n\n📈 <b>Status</b>
└─ ${statusText}`;

    message += `\n\n📊 <b>Statistics</b>
├─ 🚀 Start Balance: ${startBalance.toFixed(2)} Ks
├─ 💳 Current Balance: ${balance.toFixed(2)} Ks
└─ 📈 Total Profit: ${profitSign}${profit.toFixed(2)} Ks`;
...
└─ ${settings.pattern}`;
    ...
    if (settings.strategy === 'TREND_FOLLOW' && settings.bs_sb_wait_count > 0) {
      message += `\n\n📈 <b>TREND_FOLLOW Settings</b>
└─ BS/SB Wait: ${settings.bs_sb_wait_count}`;
   }

    if (settings.strategy === 'ALTERNATE' && settings.bb_ss_wait_count > 0) {
      message += `\n\n🔄 <b>ALTERNATE Settings</b>
└─ BB/SS Wait: ${settings.bb_ss_wait_count}`;
    }

    if (settings.strategy === 'SNIPER' && settings.sniper_state) {
      const hitCount = settings.sniper_state.hit_count || 0;
      message += `\n\n🔫 <b>SNIPER Settings</b>
├─ Hits: ${hitCount}/2
└─ Status: ${settings.sniper_state.active ? '🟢 Active' : '🔴 Waiting'}`;
    }

    if (settings.strategy === 'BEATRIX' && settings.beatrix_state) {
      const waiting = settings.beatrix_state.waiting_for_seven;
      message += `\n\n👑 <b>BEATRIX Settings</b>
└─ Status: ${waiting ? '⏳ Waiting for 7' : '✅ Ready to bet'}`;
    }

    if (settings.time_range_start && settings.time_range_end) {
      message += `\n\n⏰ <b>Time Range</b>
└─ ${settings.time_range_start} - ${settings.time_range_end}`;
    }

    message += `\n\n━━━━━━━━━━━━━━━━━
📱 Bot Version: 1.1
👨‍💻 Developer: @BLACK12566`;

    await sendPremiumMessage(ctx, message, makeAutoBetKeyboard());
...
async function textMessageHandler(ctx) {
  ...
  if (rawText.includes("📺 Watch Tutorial") || rawText.includes("Watch Tutorial")) {
    try {
      ...
      logging.error(`Error sending tutorial video: ${error.message}`);
      await sendPremiumMessage(ctx, "❌ Error sending tutorial video");
    }
    return;
  }
    ...
    if (lines.length >= 3 && lines[0].toLowerCase() === "login") {
      ...
      if (session) {
        ...
        if (userInfo && userInfo.user_id) {
          ...
          await sendPremiumMessage(ctx, userInfoMessage, makeMainMenuKeyboard(true));
  ...
  if (currentState === "INPUT_VIRTUAL_BALANCE") {
    ...
    await sendPremiumMessage(ctx, `🖥️ Switched to Virtual Mode with ${balance} Ks`, makeAutoBetKeyboard());
    delete userState[userId];
    saveUserSettings();
    return;
  }
  ...
  try {
    ...
    if (rawText === "🍎 Deactivate" || rawText === "Stop") {
      ...
      const settings = userSettings[userId] || {};
      if (!settings.running) {
        await sendPremiumMessage(ctx, "Bot is not running!", makeAutoBetKeyboard());
        return;
      }
    ...
    if (currentState === "INPUT_BET_WRAGER") {
      ...
      if (isNaN(stopLoss) || stopLoss <= 0) {
        await sendPremiumMessage(ctx, "❌ Invalid amount. Please enter a positive number for stop loss limit:\n\nExample: 10000", makeRiskControlKeyboard());
        return;
      }
    ...
    await sendPremiumMessage(ctx, `❌ Error: ${error.message}`, makeAutoBetKeyboard());
  }
}
